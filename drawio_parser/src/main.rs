...
use crate::gather::{
    gather_boards, gather_connectors, gather_harnesses, gather_interconnects, gather_sections,
};
...
fn parse_cell(e: &BytesStart, fallback_id: Option<String>) -> Cell {
    let vertex = attr_value(e, b"vertex").as_deref() == Some("1");
    let edge = attr_value(e, b"edge").as_deref() == Some("1");

    let maybe_id = attr_value(e, b"id");
    let id: String = match (maybe_id, fallback_id) {
    ...
    Cell {
        ...
        source: attr_value(e, b"source"),
        target: attr_value(e, b"target"),
        parent: attr_value(e, b"parent"),
        geometry: None,
        object: None,
    }
}
fn insert_unique_cell(map: &mut HashMap<String, Cell>, id: String, cell: Cell) {
...
pub fn parse_diagram(xml_str: &str) -> HashMap<String, Cell> {
        ...
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Start(ref e)) if e.name().as_ref() == b"mxCell" => {
                ...
                let mut cell = parse_cell(
                    e,
                    current_object.as_ref().and_then(|o| o.get("id")).cloned(),
                );
            ...
            Ok(Event::End(ref e)) if e.name().as_ref() == b"mxCell" => {
                if let Some(cell) = current_cell.take() {
                    insert_unique_cell(&mut cells, cell.id.clone(), cell);
                }
                current_cell = None;
            }
...
fn main() {
    ...
    let all_cells: HashMap<String, Cell> = parse_diagram(&xml);
    ...
    let all_harnesses = gather_harnesses(&all_cells, &all_connectors, &all_interconnects);
